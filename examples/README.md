...
> empty. See [CLI page → Test a local skill in one command](../docs/cli.md#test-a-local-skill-in-one-command).
...
Matrices: the worked matrix config is `matrices/csv-metrics-matrix.yaml` — see the `run --matrix` bullet under [CLI page → Commands at a glance](../docs/cli.md#commands-at-a-glance) ("Flags worth knowing").
