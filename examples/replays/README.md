...
> Assumes the `cowork-harness` CLI is available — from a source checkout run `npm ci && npm run build && npm link` first, or `npm i -g "cowork-harness@^3.1.0"`. (`replay` itself needs nothing else — no token, no Docker.)
