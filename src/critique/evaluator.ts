...
import { ROOT_REFERENCE_SECTION_PREFIX } from "./package-evidence.js";
...
- the transcript shows the agent guessing, backtracking, or asking a question that SKILL.md or a
  references/ file already appears to answer (check the "SKILL.md", "references/ available" and
  "${ROOT_REFERENCE_SECTION_PREFIX}" sections);
...
and "${ROOT_REFERENCE_SECTION_PREFIX}" sections), not whether the agent happened to manage without it. The agent succeeding is not evidence that
...
- "already-covered": SKILL.md, a references/ file, or a "${ROOT_REFERENCE_SECTION_PREFIX}" section (see
  those sections) already covers this; the agent overlooked or didn't act on existing guidance. A
...
A "the skill never says X" / missing-guidance complaint is judged by whether the SKILL.md, references and
"${ROOT_REFERENCE_SECTION_PREFIX}" sections shown actually contain X: if they don't, it is "grounded-and-actionable" (a real gap); if they do,
