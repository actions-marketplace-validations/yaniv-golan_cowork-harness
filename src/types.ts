...
export const Assertion = z.strictObject({
  ...
  no_scratchpad_leak: z
    ...
    .optional()
    .describe(
      "every file presented via present_files that was in the scratchpad was successfully promoted to outputs (none left behind); vacuous pass if nothing was presented — pair with a presence check to require a delivery; content-class (re-derived from the tool_use/tool_result stream, so checkable on replay too); CONTAINER TIER ONLY — the HARNESS serves present_files only at container (a scoping choice, not absence in real Cowork, which advertises it in both its VM and host-loop modes), so on hostloop/microvm a scratchpad-delivered file is neither promoted nor detected (use fidelity: container for present_files-based delivery). Production's host-loop mode validates and passes paths through WITHOUT promoting, so the promotion/leak semantics this key asserts are container-shaped even in principle. present_files is the DESKTOP-LOCAL lane's tool; remote Cowork delivers via the agent-native SendUserFile instead, so a skill should describe the delivery outcome rather than naming either tool (docs/fidelity-gaps.md, 'File delivery'); only `true` is valid",
    ),
  present_files_called: z
    .literal(true)
    .optional()
    .describe(
      "at least one file was actually delivered via the present_files tool (presentedFiles is non-empty). The presence companion to no_scratchpad_leak (which passes vacuously when nothing was presented) — pair them to require a delivery AND require it not to leak; CONTAINER TIER ONLY — the HARNESS serves present_files only at container (a scoping choice; real Cowork advertises the tool in both its VM and host-loop modes, so this is a harness coverage gap at hostloop — see docs/fidelity-gaps.md, 'File delivery'). present_files is the DESKTOP-LOCAL lane's tool name; remote Cowork uses the agent-native SendUserFile (docs/fidelity-gaps.md, 'File delivery') — this key asserts the harness-side delivery record either way; only `true` is valid",
    ),
