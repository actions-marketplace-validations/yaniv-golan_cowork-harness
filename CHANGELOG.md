...
- **A live pass was run against `desktop-2.2553.1` / agent 2.1.275** — a partial one, and DESIGN.md says so: `npm run test:live` 19 tests, 18 passed, 1 failed, **0 skipped** (the previous pass had one skip — the hostloop `critique` case — which this pass exercised for the first time and which found the transport defect fixed above); `run examples/scenarios/` 7/7 on its first run. The one live red is a pre-existing `live-matrix` case on old baselines where the model sometimes answers as text instead of calling `AskUserQuestion` — model variance, re-run and flipped, logged for hardening. `boundary-check` and the e2e self-tests were not run this pass and their previous figures are not restated as current.
