...
| `question_asked: <regex>` | the agent asked an AskUserQuestion whose **question text** matches. ⚠️ **Text only** — the gate keys below assert *that* a question was asked, how many, and whether the answer was delivered. None of them can assert the **option set or its order**, so "the founder was shown these three choices, in this order, with Stop first" is currently unassertable; the options *are* recorded (`result.json` → `decisions[].questions[].options[]`, in the order offered), so check them by hand when the presented choice matters |
