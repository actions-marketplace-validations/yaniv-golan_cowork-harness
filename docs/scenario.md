...
| `tool_not_called: <glob>` | no tool the agent ran matched this glob (`mcp__*` = no MCP tool ran) — same load-time reject on an empty or regex-like glob as `tool_called`, and the same legacy-name matching. ⚠️ **This key can still pass VACUOUSLY when the tier does not offer the tool at all** (e.g. `Bash` at `hostloop`, where it is replaced by `mcp__workspace__bash`; or `mcp__workspace__bash` at `container`, where the built-in `Bash` is served instead) — the run could never have called it, so the assertion verifies nothing. Check the tier's tool surface before relying on a green here |
