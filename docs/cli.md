...
```bash
npm install -g "cowork-harness@^3.6.0"    # puts the `cowork-harness` command on your PATH
```
...
> From a global install (`npm i -g "cowork-harness@^3.6.0"`), point at the package root instead:
...
> - **Invocation:** from a source checkout, `node dist/cli.js <cmd>` (or `npm link` to get the `cowork-harness` command); from a global install, `cowork-harness <cmd>`; the companion skill falls back to `npx "cowork-harness@^3.6.0"`.
...
The canonical package-contents table. `package.json`'s `files` field is the machine source; this is the
readable form.
...
`scenario.py`/assertion keys, per `.claude-plugin/marketplace.json`'s `source`); the rest of the table
arrives when the skill's first command self-bootstraps `npx "cowork-harness@^3.6.0"`. See
...
|---|---|---|
| `skill <folder> "<prompt>"` | Run a local skill/plugin folder once against the staged agent. An unanswered gate's `--on-unanswered` defaults adaptively (`prompt` on a TTY, `fail` in CI). Ablation, labelling, repeat and the three consent flags: see [Flags worth knowing](#flags-worth-knowing) | ad-hoc "is the skill alive / does it do X?" — the fast inner loop |
...
| `lint-skill <SKILL.md \| skill-dir/>…` | Lint a skill body (and any sibling `hooks.json`) for two Cowork host-loop footguns, resolve pinned `subagent_type` values against the enclosing plugin, and size the skill corpus against the `critique` evidence ceiling. Severities and `--strict`: see [Flags worth knowing](#flags-worth-knowing) | authoring or reviewing a skill before a paid Cowork host-loop run exposes the footgun, or before a pinned `subagent_type` typo breaks a `Task` dispatch |
...
| `stats [<scenario>]` | Queryable summary over every indexed `run`/`skill`/`record` invocation — run count, pass rate, cost/duration/token/turn percentiles, group spend, last-green timestamp. Grouping and filters: see [Flags worth knowing](#flags-worth-knowing) and [docs/stats.md](./stats.md) | "is this scenario flaky/expensive over time?" without hand-aggregating `result.json` files |
