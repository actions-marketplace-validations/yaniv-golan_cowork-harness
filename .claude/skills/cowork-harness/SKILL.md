...
metadata:
  ...
  version: 1.21.1
  tracks-harness: cowork-harness 1.21.1 (baseline desktop-1.26832.0)
---
...
> **Version note:** the facts and `file:line` pointers here track `cowork-harness 1.21.1` (baseline
...
- **CLI on PATH, recent enough?** Run `cowork-harness --version` — this skill needs **≥ 1.21.1**. If it's missing or older, prefix every command with the version floor `npx "cowork-harness@>=1.21.1" <cmd>` (Node ≥ 22), or install once with `npm i -g "cowork-harness@>=1.21.1"`. **Pin `@>=1.21.1`, never `@latest`** — `@latest` can silently fetch an older CLI and the new commands fail as "unknown command", whereas the floor **fails loud** if no compatible version is published.
