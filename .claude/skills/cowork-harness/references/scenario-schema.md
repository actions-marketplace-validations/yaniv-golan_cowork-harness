...
| **residual vacuity** | `tool_not_called` still passes vacuously when the TIER never offers the tool: `Bash` at `hostloop` (replaced by `mcp__workspace__bash`) and `mcp__workspace__bash` at `container` (the built-in is served instead). The run could not have called it, so the assertion verifies nothing. Check the tier's surface. |
