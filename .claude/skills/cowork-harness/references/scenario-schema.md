...
| `undelivered_deliverables` | warn | The skill produced file(s) OUTSIDE every user-visible root and never delivered them — on a remote Cowork session the workspace is reclaimed at session end; on a local one they stay invisible to the user. Fires on every run without opting in, because the scenarios that most need it are the ones whose author never considered delivery. Silent when the evidence cannot answer the question (no workspace walk, or a tier that runs no scratchpad walk) — never a vacuous clean. No opt-out key; write deliverables under `outputs/` or deliver them explicitly |
