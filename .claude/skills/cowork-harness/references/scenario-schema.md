...
| `question_asked: <regex>` | the agent asked an AskUserQuestion whose **question text** matches. ⚠️ Text only — no gate key asserts the **option set or its order**, so "these three choices, in this order, Stop first" is unassertable today; the options *are* recorded in order at `result.json` → `decisions[].questions[].options[]`, so check them by hand when what the user was *shown* is the thing under test |
